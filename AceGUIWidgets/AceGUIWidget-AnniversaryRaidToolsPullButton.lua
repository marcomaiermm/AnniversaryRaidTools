...
local Type, Version = "ARTPullButton", 1
...
local function GetDropTarget()
  local scrollFrame = ART.main_frame.sidePanel.pullButtonsScrollFrame
  local buttonList = ART.main_frame.sidePanel.newPullButtons
  ...
  if scrollFrame.frame:IsMouseOver(1, -1, -dragdrop_overlap, dragdrop_overlap) then
    ...
    if not id then
      local addNewPullButton = ART.main_frame.sidePanel.newPullButton
      if addNewPullButton.frame:IsMouseOver(2) then
        local maxPulls = #ART:GetCurrentPreset().value.pulls
        id = maxPulls
...
local methods = {
  ...
  ["Initialize"] = function(self)
      ...
      ART:CreateContextMenu(ART.main_frame, function(ownerRegion, rootDescription)
        if buttonSelf.index ~= 1 then
          rootDescription:CreateButton(L["Pull Drop Move up"], function()
            ART:MovePullUp(buttonSelf.index)
        ...
        if buttonSelf.index < buttonSelf.maxPulls then
          rootDescription:CreateButton(L["Pull Drop Move down"], function()
            ART:MovePullDown(buttonSelf.index)
            if ART.liveSessionActive and ART:GetCurrentPreset().uid == ART.livePresetUID then
              ART:LiveSession_SendPulls(ART:GetPulls())
            end
          end)
        ...
        rootDescription:CreateButton(L["Pull Drop Insert before"], function()
          ART:PresetsAddPull(self.index)
          ART:ReloadPullButtons()
          ART:SetSelectionToPull(self.index)
          if ART.liveSessionActive and ART:GetCurrentPreset().uid == ART.livePresetUID then
            ART:LiveSession_SendPulls(ART:GetPulls())
          end
        end)
        ...
        rootDescription:CreateButton(L["Pull Drop Insert after"], function()
          ART:PresetsAddPull(buttonSelf.index + 1)
          ART:ReloadPullButtons()
          ART:SetSelectionToPull(buttonSelf.index + 1)
          if ART.liveSessionActive and ART:GetCurrentPreset().uid == ART.livePresetUID then
            ART:LiveSession_SendPulls(ART:GetPulls())
          end
        end)
        ...
        if buttonSelf.index ~= 1 then
          rootDescription:CreateButton(L["Pull Drop Merge up"], function()
            local newIndex = ART:PresetsMergePulls(buttonSelf.index, buttonSelf.index - 1)
            ART:ReloadPullButtons()
            ART:SetSelectionToPull(newIndex)
            if ART.liveSessionActive and ART:GetCurrentPreset().uid == ART.livePresetUID then
              ART:LiveSession_SendPulls(ART:GetPulls())
            end
          end)
    ...
    local function openMultiContextMenu()
      ART:CreateContextMenu(ART.main_frame, function(ownerRegion, rootDescription)
        ...
        rootDescription:CreateButton(L["Pull Drop Insert after"], function()
          ...
          ART:PresetsAddPull(self.index + 1)
          ART:ReloadPullButtons()
          ART:SetSelectionToPull(self.index + 1)
          if ART.liveSessionActive and ART:GetCurrentPreset().uid == ART.livePresetUID then
            ART:LiveSession_SendPulls(ART:GetPulls())
          end
        end)
        ...
        rootDescription:CreateButton(L["Pull Drop Merge"], function()
          local selected_pulls = CopyTable(ART:GetSelection())
          -- Assure, that the destination is always the last selected_pull, to copy it's options at last
          ART.U.iremove_if(selected_pulls, function(pullIdx)
            return pullIdx == self.index
          end)

          if not ART.U.contains(selected_pulls, self.index) then
            tinsert(selected_pulls, self.index)
          end

          local newIndex = ART:PresetsMergePulls(selected_pulls, self.index)
          ART:ReloadPullButtons()
          ART:GetCurrentPreset().value.selection = { newIndex }
          ART:SetSelectionToPull(newIndex)
          if ART.liveSessionActive and ART:GetCurrentPreset().uid == ART.livePresetUID then
            ART:LiveSession_SendPulls(ART:GetPulls())
          end
        ...
        rootDescription:CreateButton(L["Pull Drop Clear Pulls"], function()
          if not ART.U.contains(ART:GetSelection(), self.index) then
            tinsert(ART:GetSelection(), self.index)
            self:Pick()
          end

          for _, pullIdx in ipairs(ART:GetSelection()) do
            ART:ClearPull(pullIdx)
          end
          if ART.liveSessionActive and ART:GetCurrentPreset().uid == ART.livePresetUID then
            ART:LiveSession_SendPulls(ART:GetPulls())
          end
        ...
        if self.maxPulls > 1 then
          rootDescription:CreateButton(L["Pull Drop Delete"], function()
            local addPull = false
            local button = ART:GetFirstNotSelectedPullButton(self.index, "UP")
            if not button then
              button = ART:GetFirstNotSelectedPullButton(self.index, "DOWN")
              if not button then
                addPull = true
                button = 1
              end
            ...
            local removed_pulls = {}
            for _, pullIdx in pairs(ART.GetSelection()) do
              local offset = ART.U.count_if(removed_pulls, function(entry)
                return entry < pullIdx
              end)

              ART:DeletePull(pullIdx - offset)
              tinsert(removed_pulls, pullIdx)
            end

            ART.GetCurrentPreset().value.selection = {}

            if not addPull then
              local offset = ART.U.count_if(removed_pulls, function(entry)
                return entry < button
              end)
              ART:SetSelectionToPull(button - offset)
            else
              --ART:AddPull(1) --we handle not deleting all pulls in ART:DeletePull() instead
    ...
    function self.callbacks.OnClickNormal(_, mouseButton, force)
      if not force and not ART.main_frame.sidePanel.pullButtonsScrollFrame.frame:IsMouseOver() then return end
      ...
      if (IsControlKeyDown()) then
        if (mouseButton == "LeftButton") then
          ...
          if not ART.U.contains(ART:GetSelection(), self.index) then
            tinsert(ART:GetSelection(), self.index)
            local changed = ART:SetMapSublevel(self.index)
            ART:SetSelectionToPull(ART:GetSelection())
            if changed then ART:UpdateMap() end
          else
            ART.U.iremove_if(ART:GetSelection(), function(entry)
              return entry == self.index
            end)
            self:ClearPick()
          end
      ...
      elseif (IsShiftKeyDown()) then
        if (mouseButton == "LeftButton") then
          ...
          for i = lastPull, self.index, step do
            if not ART.U.contains(selection, i) then
              tinsert(selection, i)
            end
          end
          local changed = ART:SetMapSublevel(self.index)
          ART:SetSelectionToPull(selection)
          if changed then ART:UpdateMap() end
        elseif (mouseButton == "RightButton") then
          local maxPulls = #ART:GetCurrentPreset().value.pulls
          if maxPulls > 1 then
            ART:DeletePull(self.index)
          end
        end
      else
        ART:EnsureDBTables()
        if (mouseButton == "RightButton") then
          ...
          --normal click
          ART:GetCurrentPreset().value.selection = { self.index }
          local changed = ART:SetMapSublevel(self.index)
          ART:SetSelectionToPull(self.index)
          local shouldAutoPan = ART:GetDB().autoPanToPull ~= false
          if changed then
            if shouldAutoPan then ART.pendingAutoPanToPull = self.index end
            ART:UpdateMap()
          elseif shouldAutoPan then
            ART:PanMapToPull(self.index)
          end
        end
      end
    end
    ...
    function self.callbacks.OnEnter()
      ...
      for _, button in pairs(ART.main_frame.sidePanel.newPullButtons) do
        if button.dragging then return end
      end
      ART.pullTooltip:SetPoint("TOPRIGHT", self.frame, "TOPLEFT", 0, 0)
      ART.pullTooltip:SetPoint("BOTTOMRIGHT", self.frame, "TOPLEFT", -250, -(4 + ART.pullTooltip.myHeight))
      local tooltipBottom = ART.pullTooltip:GetBottom()
      local mainFrameBottom = ART.main_frame:GetBottom()
      if tooltipBottom < mainFrameBottom then
        ART.pullTooltip:SetPoint("TOPRIGHT", self.frame, "BOTTOMLEFT", 0, (4 + ART.pullTooltip.myHeight))
        ART.pullTooltip:SetPoint("BOTTOMRIGHT", self.frame, "BOTTOMLEFT", -250, -4)
      end
      self.entered = true
      ART:ActivatePullTooltip(self.index)
      self.frame:SetScript("OnUpdate", self:CreateUpdateFunction())
      ART:PullClickAreaOnEnter(self.index)
    end

    function self.callbacks.OnLeave()
      ART.pullTooltip.Model:Hide()
      ART.pullTooltip.topString:Hide()
      self.entered = false
      self.frame:SetScript("OnUpdate", nil)
      ART:UpdatePullTooltip(ART.pullTooltip)
    ...
    if not self.updateFunction then
      self.updateFunction = function(frame, elapsed)
        ...
        if self.dragging then
          if ART.pullTooltip:IsShown() then
            ART.pullTooltip:Hide()
          end
          ...
          if scrollFrame.frame:IsMouseOver(scroll_hover.height, height - scroll_hover.offset, -dragdrop_overlap,
            ...
            if self.top_hover > scroll_hover.timeout then
              ...
              local scroll_pulls = ART.U.lerp(scroll_hover.pulls_per_second.min, scroll_hover.pulls_per_second.max,
                scroll_speed)
              local scroll_pixel = scroll_pulls * self.frame:GetHeight()
              local scroll_amount = ART:GetScrollingAmount(scrollFrame, scroll_pixel) * scroll_hover.timeout
          ...
          if self.elapsed > 0.1 then
            ...
            ART:Show_DropIndicator(button, pos)
            self.elapsed = 0
          end
        end
      end
  ...
  ["Drag"] = function(self)
    ...
    if #ART:GetSelection() > 1 then
      if not ART.U.contains(ART:GetSelection(), self.index) then
        ...
        ART:GetCurrentPreset().value.currentPull = self.index
        ART:GetCurrentPreset().value.selection = { self.index }
        self:Pick()
      end
  ...
  ["Drop"] = function(self)
    ...
    if #ART:GetSelection() > 1 then
      ...
      for offset, pullIdx in ipairs(selected_pulls) do
        ...
        local progressed_above = ART.U.count_if(progressed_pulls, function(entry)
          return entry < pos
        end)
        ...
        if pos > correctPullIndex then
          correctPullIndex = correctPullIndex - ART.U.count_if(progressed_pulls, function(entry)
        ...
        ART:PresetsAddPull(pos)
        ART:CopyPullOptions(correctPullIndex, pos)
        local newID = ART:PresetsMergePulls(correctPullIndex, pos)
        --print("newID", newID)
      ...
      ART:GetCurrentPreset().value.selection = new_pulls
      ART:ReloadPullButtons()
      ART:SetSelectionToPull(1)
    else
      ...
      ART:PresetsAddPull(insertID)
      ART:CopyPullOptions(index, insertID)
      local newIndex = ART:PresetsMergePulls(index, insertID)
      ART:ReloadPullButtons()
      ART:SetSelectionToPull(newIndex)
    end

    ART:Hide_DropIndicator()
    ART.pullTooltip:Show()
    if ART.liveSessionActive and ART:GetCurrentPreset().uid == ART.livePresetUID then
      ART:LiveSession_SendPulls(ART:GetPulls())
    end
  end,
  ...
  ["SetIndex"] = function(self, index)
    ...
    self.color.r, self.color.g, self.color.b = ART:RaidEnemies_GetPullColor(self.index)
    self:UpdateColor()
  end,
  ...
  ["UpdateColor"] = function(self)
    local colorHex = ART:RGBToHex(self.color.r, self.color.g, self.color.b)
  ...
  ["UpdateCountText"] = function(self)
    local db = ART:GetDB()
    local pullHealth = db.showPullButtonHealth and ART:SumCurrentPullHealth(self.index) or 0
    local text = pullHealth > 0 and ART:FormatEnemyHealth(pullHealth) or ""
    if text ~= "" then
      self.percentageFontString:SetText(text)
      self.percentageFontString:SetHeight(10)
...
local function Constructor()
  local name = "ARTPullButton"..AceGUI:GetNextWidgetNum(Type);
  ...
  pickedGlow:SetTexture("Interface\\AddOns\\AnniversaryRaidTools\\Textures\\HeartOfAzerothSelection")
  ...
  for i = 1, maxPortraitCount do
    ...
    enemyPortraits[i].overlay:SetTexture("Interface\\Addons\\AnniversaryRaidTools\\Textures\\Circle_White")
