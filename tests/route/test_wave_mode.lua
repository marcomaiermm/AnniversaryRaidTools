...
_G.ART = ART
local addon = { ART = ART, L = setmetatable({}, { __index = function(_, key) return key end }) }
...
local MDT = addon
MDT.mapInfo = { [162] = { mapID = 534 } }
function MDT:GetDB() return { currentDungeonIdx = 162 } end
function MDT:GetCurrentPreset() return current end
function MDT:GetDefaultMapPanelSize() return 1000, 1000 end
function MDT:SetSelectionToPull(index) current.value.currentPull = index end
function MDT:UpdateMap() end
function MDT:MakePullSelectionButtons() end

MDT.dungeonEnemies = { [162] = {} }
ART.MultiRaidIntegration = { spawnLookup = { hyjal = {} } }
local enemyIdx = 0
for _, enemy in pairs(raid.enemies) do
  enemyIdx = enemyIdx + 1
  local clones = {}
  MDT.dungeonEnemies[162][enemyIdx] = { clones = clones }
...
for _, spawn in ipairs(raid.enemies["17895"].spawns) do
  ...
  MDT.dungeonEnemies[162][reference.enemyIdx].clones[reference.cloneIdx].patrol = {
    { x = 61, y = -742 }, { x = 118, y = -681 },
  }
end

assert(loadfile(root.."/Modules/WaveModeUI.lua"))("AnniversaryRaidTools", MDT)
...
widgetText = "Wave 6/8"
ui:ResetHyjalRuntime()
MDT.main_frame = { IsShown = function() return true end }
function MDT:IsMapSectionActive() return true end
...
local enemies = assert(io.open(root.."/Modules/DungeonEnemies.lua", "rb"))
...
local core = assert(io.open(root.."/MythicDungeonTools.lua", "rb"))
local coreSource = core:read("*a")
core:close()
assert(coreSource:find("if preset.value.artWaveRaid then", 1, true)
    and coreSource:find("MDT:ReleaseHullTextures()", 1, true), "wave raids must suppress pull hulls")
