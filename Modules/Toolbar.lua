...
local db
local toolbarTools = {}
local activeTextures = ART.presetObjectActiveTextures
local currentTool
...
function ART:initToolbar(frame)
  db = ART:GetDB()

  frame.toolbar = CreateFrame("Frame", "ARTToolbarFrame", frame)
  ...
  frame.toolbar.tex:SetColorTexture(unpack(ART.BackdropColor))
  ...
  frame.toolbar.toggleButton:SetNormalTexture(ART.AddonPath.."Textures\\arrows")
  frame.toolbar.toggleButton:GetNormalTexture():SetTexCoord(0.7, 1, 0, 1)

  frame.toolbar:Hide()
  frame.toolbar:SetScript("OnHide", function()
    ART:UpdateSelectedToolbarTool(nil)
  end)
  ...
  ART:FixAceGUIShowHide(frame.toolbar.widgetGroup, frame.toolbar)
  ...
  local widgets = {}
  ART.tempWidgets = widgets

  ---back
  local back = AceGUI:Create("Icon")
  back:SetImage(ART.AddonPath.."Textures\\icons", 0.5, 0.75, 0.5, 0.75)
  ...
  forward:SetImage(ART.AddonPath.."Textures\\icons", 0.75, 1, 0.5, 0.75)
  ...
  plus:SetImage(ART.AddonPath.."Textures\\icons", 0.25, 0.5, 0.5, 0.75)
  ...
  minus:SetImage(ART.AddonPath.."Textures\\icons", 0, 0.25, 0.5, 0.75)
  ...
  pencil:SetImage(ART.AddonPath.."Textures\\icons", 0, 0.25, 0, 0.25)
  toolbarTools["pencil"] = pencil
  pencil:SetCallback("OnClick", function(widget, callbackName)
    if currentTool == "pencil" then ART:UpdateSelectedToolbarTool() else ART:UpdateSelectedToolbarTool("pencil") end
  end)
  ...
  line:SetImage(ART.AddonPath.."Textures\\icons", 0, 0.25, 0.75, 1)
  toolbarTools["line"] = line
  line:SetCallback("OnClick", function(widget, callbackName)
    if currentTool == "line" then ART:UpdateSelectedToolbarTool() else ART:UpdateSelectedToolbarTool("line") end
  end)
  ...
  arrow:SetImage(ART.AddonPath.."Textures\\icons", 0.25, 0.5, 0, 0.25)
  toolbarTools["arrow"] = arrow
  arrow:SetCallback("OnClick", function(widget, callbackName)
    if currentTool == "arrow" then ART:UpdateSelectedToolbarTool() else ART:UpdateSelectedToolbarTool("arrow") end
  end)
  ...
  note:SetImage(ART.AddonPath.."Textures\\icons", 0.75, 1, 0, 0.25)
  toolbarTools["note"] = note
  note:SetCallback("OnClick", function(widget, callbackName)
    if currentTool == "note" then ART:UpdateSelectedToolbarTool() else ART:UpdateSelectedToolbarTool("note") end
  end)
  ...
  mover:SetImage(ART.AddonPath.."Textures\\icons", 0.5, 0.75, 0, 0.25)
  toolbarTools["mover"] = mover
  mover:SetCallback("OnClick", function(widget, callbackName)
    if currentTool == "mover" then ART:UpdateSelectedToolbarTool() else ART:UpdateSelectedToolbarTool("mover") end
  end)
  ...
  eraser:SetImage(ART.AddonPath.."Textures\\icons", 0.25, 0.5, 0.25, 0.5)
  toolbarTools["eraser"] = eraser
  eraser:SetCallback("OnClick", function(widget, callbackName)
    if currentTool == "eraser" then ART:UpdateSelectedToolbarTool() else ART:UpdateSelectedToolbarTool("eraser") end
  end)
  ...
  delete:SetImage(ART.AddonPath.."Textures\\icons", 0.25, 0.5, 0.75, 1)
  ...
  for k, widget in ipairs(widgets) do
    ...
    widget:SetCallback("OnEnter", function(widget, callbackName)
      ART:ToggleToolbarTooltip(true, widget)
    end)
  ...
  ART:CreateBrushPreview(frame)
  ART:UpdateSelectedToolbarTool()
  frame.toolbar.widgetGroup.frame:Hide()
end

---CreateBrushPreview
function ART:CreateBrushPreview(frame)
  frame.brushPreview = CreateFrame("Frame", "AnniversaryRaidToolsBrushPreview", UIParent)
  ...
  frame.brushPreview.tex:SetTexture(ART.AddonPath.."Textures\\ring")
  frame.brushPreview.tex:SetAllPoints()
end

---EnableBrushPreview
function ART:EnableBrushPreview(tool)
  ...
  frame.brushPreview:SetScript("OnUpdate", function(self, tick)
    if ARTScrollFrame:IsMouseOver() and not ARTToolbarFrame:IsMouseOver() then
...
---DisableBrushPreview
function ART:DisableBrushPreview()
  local frame = ART.main_frame
...
function ART:ToggleToolbarTooltip(show, widget, anchor)
  if not show then
    GameTooltip:Hide()
  else
...
function ART:UpdateSelectedToolbarTool(widgetName)
  local toolbar = ART.main_frame.toolbar
  if not widgetName or (not toolbarTools[widgetName]) then
    if toolbar.highlight then toolbar.highlight:Hide() end
    ART:RestoreScrollframeScripts()
    ART:DisableBrushPreview()
    if ART.drawingActive then
      if currentTool == "pencil" then ART:StopPencilDrawing() end
      if currentTool == "arrow" then ART:StopArrowDrawing() end
      if currentTool == "line" then ART:StopLineDrawing() end
      if currentTool == "mover" then ART:StopMovingDrawing() end
      if currentTool == "eraser" then ART:StopEraserDrawing() end
    end
    currentTool = nil
  ...
  toolbar.highlight:SetTexture(ART.AddonPath.."Textures\\icons")
  ...
  ART:OverrideScrollframeScripts()
  ART:EnableBrushPreview(currentTool)
...
function ART:OverrideScrollframeScripts()
  local frame = ART.main_frame
  frame.scrollFrame:SetScript("OnMouseDown", function(self, button)
    if button == "LeftButton" then
      if currentTool == "pencil" then ART:StartPencilDrawing() end
      if currentTool == "arrow" then ART:StartArrowDrawing() end
      if currentTool == "line" then ART:StartLineDrawing() end
      if currentTool == "mover" then ART:StartMovingObject() end
      if currentTool == "eraser" then ART:StartEraserDrawing() end
    end
  ...
  frame.scrollFrame:SetScript("OnMouseUp", function(self, button)
    if button == "LeftButton" then
      ...
      if currentTool == "arrow" then ART:StopArrowDrawing() end
      if currentTool == "line" then ART:StopLineDrawing() end
      if currentTool == "mover" then ART:StopMovingObject() end
      if currentTool == "eraser" then ART:StopEraserDrawing() end
      if currentTool == "note" then ART:StartNoteDrawing() end
    end
    if button == "RightButton" then
      local scrollFrame = ART.main_frame.scrollFrame
  ...
  ART:SetPresetNotesMovable(currentTool == "mover")
end
...
function ART:GetHighestFrameLevelAtCursor()
...
local nobj
---StartArrowDrawing
function ART:StartArrowDrawing()
  ART.drawingActive = true
  local frame = ART.main_frame
  local startx, starty = ART:GetCursorPosition()
  local line = ART:AcquirePresetObjectTexture()
  ...
  line:Show()
  local arrow = ART:AcquirePresetObjectTexture()
  arrow:SetTexture(ART.AddonPath.."Textures\\triangle")
  ...
  nobj = {
    d = { arrowBrushSize, 1, ART:GetCurrentSubLevel(), true,
      ART:RGBToHex(db.toolbar.color.r, db.toolbar.color.g, db.toolbar.color.b) },
    l = {}
  }
  nobj.l = { ART:Round(startx, 1), ART:Round(starty, 1) }
  nobj.t = {}
  ...
  frame.toolbar:SetScript("OnUpdate", function(self, tick)
    ...
    local x, y = ART:GetCursorPosition()
    local currentDrawLayer = ART:GetHighestFrameLevelAtCursor()
    drawLayer = max(drawLayer, currentDrawLayer)
    if x ~= startx and y ~= starty then
      DrawLine(line, ART.main_frame.mapPanelTile1, startx, starty, x, y, (arrowBrushSize * 0.3) * scale, 1, "TOPLEFT")
      nobj.l[3] = ART:Round(x, 1)
      nobj.l[4] = ART:Round(y, 1)
    end
    --position arrow head
    arrow:Show()
    ...
    arrow:SetPoint("CENTER", ART.main_frame.mapPanelTile1, "TOPLEFT", x, y)
...
---StopArrowDrawing
function ART:StopArrowDrawing()
  local frame = ART.main_frame
  ART:StorePresetObject(nobj)
...
function ART:StartLineDrawing()
  ...
  local circle1 = ART:AcquirePresetObjectTexture()
  tinsert(activeTextures, circle1)
  local circle2 = ART:AcquirePresetObjectTexture()
  ...
  ---l: x1,y1,x2,y2,...
  nobj = {
    d = { db.toolbar.brushSize, 1.1, ART:GetCurrentSubLevel(), true,
      ART:RGBToHex(db.toolbar.color.r, db.toolbar.color.g, db.toolbar.color.b), nil, true },
    l = {}
  }
  nobj.l = {}
  ...
  frame.toolbar:SetScript("OnUpdate", function(self, tick)
    ...
    local currentDrawLayer = ART:GetHighestFrameLevelAtCursor()
    drawLayer = max(drawLayer, currentDrawLayer)
    endx, endy = ART:GetCursorPosition()
    if endx ~= startx and endy ~= starty then
      DrawLine(line, ART.main_frame.mapPanelTile1, startx, starty, endx, endy, (db.toolbar.brushSize * 0.3) * 1.1 * scale
      , 1.00, "TOPLEFT")
      line:SetDrawLayer(objectDrawLayer, drawLayer)
      line:Show()
      ART:DrawCircle(startx, starty, (db.toolbar.brushSize * 0.3) * scale, db.toolbar.color, objectDrawLayer, drawLayer,
        true, nil, circle1, true)
      ART:DrawCircle(endx, endy, (db.toolbar.brushSize * 0.3) * scale, db.toolbar.color, objectDrawLayer, drawLayer, true
      , nil, circle2, true)
      nobj.d[6] = drawLayer
    end
  end)
...
function ART:StopLineDrawing()
  ...
  for i = 1, numSegments do
    ...
    nobj.l[4 * i - 3] = ART:Round(x, 1)
    nobj.l[4 * i - 2] = ART:Round(y, 1)
    nobj.l[4 * i - 1] = ART:Round(newx, 1)
    nobj.l[4 * i] = ART:Round(newy, 1)
    x, y = newx, newy
  end
  tinsert(nobj.l, ART:Round(x, 1))
  tinsert(nobj.l, ART:Round(y, 1))
  tinsert(nobj.l, ART:Round(endx, 1))
  tinsert(nobj.l, ART:Round(endy, 1))
...
function ART:StartPencilDrawing()
  ART.drawingActive = true
  local frame = ART.main_frame
  oldx = nil
  oldy = nil
  ...
  nobj = {
    d = { db.toolbar.brushSize, 1.1, ART:GetCurrentSubLevel(), true,
      ART:RGBToHex(db.toolbar.color.r, db.toolbar.color.g, db.toolbar.color.b), 0, true },
    l = {}
  }
  nobj.l = {}
  ...
  frame.toolbar:SetScript("OnUpdate", function(self, tick)
    ...
    local x, y = ART:GetCursorPosition()
    local mapScale = ART.main_frame.mapPanelFrame:GetScale()
    ...
    if (oldx and abs(x - oldx) > threshold) or (oldy and abs(y - oldy) > threshold) then
      ART:DrawLine(oldx, oldy, x, y, (db.toolbar.brushSize * 0.3) * scale, db.toolbar.color, true, objectDrawLayer,
        layerSublevel, nil, true)
      nobj.d[6] = layerSublevel
      nobj.l[lineIdx] = ART:Round(oldx, 1)
      nobj.l[lineIdx + 1] = ART:Round(oldy, 1)
      nobj.l[lineIdx + 2] = ART:Round(x, 1)
      nobj.l[lineIdx + 3] = ART:Round(y, 1)
      lineIdx = lineIdx + 4
      oldx, oldy = x, y
    end
  end)
...
function ART:StopPencilDrawing()
  local frame = ART.main_frame
  local x, y = ART:GetCursorPosition()
  local layerSublevel = ART:GetHighestFrameLevelAtCursor()
  local scale = ART:GetScale()
  --finish line
  if x ~= oldx or y ~= oldy then
    ...
    nobj.l[size + 1] = ART:Round(oldx, 1)
    nobj.l[size + 2] = ART:Round(oldy, 1)
    nobj.l[size + 3] = ART:Round(x, 1)
    nobj.l[size + 4] = ART:Round(y, 1)
  end
  frame.toolbar:SetScript("OnUpdate", nil)
  --clear own flags
  for k, v in pairs(activeTextures) do
  ...
  if lineCount > 0 then
    ...
    ART:DrawCircle(x, y, db.toolbar.brushSize * 0.3 * scale, db.toolbar.color, objectDrawLayer, layerSublevel)
    ART:StorePresetObject(nobj)
...
---StartNoteDrawing
function ART:StartNoteDrawing()
  ---new object for storage
  ---x,y,sublevel,shown,text,n=true
  local x, y = ART:GetCursorPosition()
  nobj = { d = { x, y, ART:GetCurrentSubLevel(), true, "" } }
  nobj.n = true
  ART:StorePresetObject(nobj)
  if self.liveSessionActive then self:LiveSession_SendObject(nobj) end
  ART:DrawAllPresetObjects()
